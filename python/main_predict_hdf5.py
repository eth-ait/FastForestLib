...
def run(forest_file, test_data_file, config, prediction_output_file=None, output_gt=False, profiler=None):
    print("Loading forest predictor ...")
    predictor = image_weak_learner.Predictor.read_from_matlab_file(forest_file)
    print("Done")

    if 'num_of_samples_per_image' not in config.testing_data_parameters \
            or config.testing_data_parameters['num_of_samples_per_image'] <= 0:
        test_data = ImageDataReader.read_from_matlab_file_with_all_samples(test_data_file,
                                                                            **config.testing_data_parameters)
    else:
        test_data = ImageDataReader.read_from_matlab_file_with_random_samples(test_data_file,
                                                                                **config.testing_data_parameters)
    ...
    if prediction_output_file is not None:
        # output_mat_dict = {}
        output_mat = h5py.File(prediction_output_file, 'w')
    ...
    if prediction_output_file is not None:
        ...
        if output_gt:
            pixel_gt_labels = - np.ones((test_data.num_of_images, test_data.image_width * test_data.image_height), dtype=np.int64)
    for i in xrange(test_data.num_of_images):
        ...
        if prediction_output_file is not None:
            ...
            if output_gt:
                pixel_gt_labels[i, sample_indices] = flat_labels[sample_indices]
    ...
    if prediction_output_file is not None:
        ...
        output_mat.create_dataset('pixel_predicted_labels', data = pixel_predicted_labels, compression = "gzip", compression_opts = 9)
        output_mat.create_dataset('pixel_confusion_matrix', data = confusion_matrix, compression = "gzip", compression_opts = 9)
        if output_gt:
            pixel_gt_labels = pixel_gt_labels.reshape((test_data.num_of_images, test_data.image_width, test_data.image_height))
            output_mat.create_dataset('pixel_gt_labels', data = pixel_gt_labels, compression = "gzip", compression_opts = 9)
    ...
    if prediction_output_file is not None:
        ...
        if output_gt:
            frame_gt_labels = - np.ones((test_data.num_of_images,), dtype=np.int64)
    for i in xrange(test_data.num_of_images):
        # print("Testing image {}".format(i + 1))
        image = test_data.data[i, :, :]
        labels = test_data.labels[i, :, :]
        ...
        if prediction_output_file is not None:
            ...
            if output_gt:
                frame_gt_labels[i] = frame_gt_label

    if prediction_output_file is not None:
        # output_mat_dict['frame_predicted_labels'] = frame_predicted_labels
        # output_mat_dict['frame_confusion_matrix'] = confusion_matrix
        output_mat.create_dataset('frame_predicted_labels', data = frame_predicted_labels, compression = "gzip", compression_opts = 9)
        output_mat.create_dataset('frame_confusion_matrix', data = confusion_matrix, compression = "gzip", compression_opts = 9)
        if output_gt:
            output_mat.create_dataset('frame_gt_labels', data = frame_gt_labels, compression = "gzip", compression_opts = 9)
    ...
    #     scipy.io.savemat(prediction_output_file, output_mat_dict, do_compression = True)
    #     print("Done.")
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) < 3:
        print("Usage: python {} <test data file> <forest file> [configuration file] [prediction output file] [output ground truth]".format(sys.argv[0]))
        sys.exit(1)
    ...
    run(forest_file, test_data_file, config, prediction_output_file, output_gt=output_gt)
